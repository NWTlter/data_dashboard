...
create_seasonal_plot <- function(seasonal_data, trend_results, target_season,
  ...
  p <- ggplot(plot_data, aes(x = year, y = !!sym(y_col), color = site)) +
    ...
    guides(
      color = guide_legend(order=1, nrow = 1, override.aes = list(size = 1.2), keywidth = 1,
                           title.theme = element_text(size = 7),  
                           label.theme = element_text(size = 7)),
      linetype = guide_legend(order=2, nrow = 3, override.aes = list(size = 1.2, linetype = c("solid", "dashed", "dotted")), keywidth = 2.5,
                              title.theme = element_text(size = 7),  
                              label.theme = element_text(size = 7))
    )
...
create_individual_snow_plot <- function(data, trend_result, y_col, y_label, plot_title, 
  ...
  main_plot <- ggplot(plot_data, aes(x = year, y = !!sym(y_col), color = site)) +
    ...
    guides(
      color = guide_legend(order = 1, nrow = 1, override.aes = list(linewidth = 1.2), keywidth = 1,
...
sig_legend_combined <- ggplot(
  ...
  theme_minimal() +
  theme(legend.position = "bottom") +
  guides(linetype = guide_legend(nrow = 3, override.aes = list(linewidth = 1.2), 
                                 keywidth = 2.5,
